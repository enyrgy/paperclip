...
- [ ] **FlexOffers: wire the remaining lead magnets (4 of 5 done)** — STRUCTURE: one program (250377) with a **campaign per lead magnet**, since the destination URL lives at the campaign level while tracking, payout, and publisher approvals sit at the program level. A publisher applies once and can then promote whichever magnet fits their audience. Per magnet the work is: (1) add the refid capture code to the page, (2) one test submission so GHL learns the field, (3) map `flexoffers_refid` in that magnet's capture workflow, (4) create its FlexOffers campaign + creative. Only step 4 is blocked on FlexOffers.
  ...
  - [ ] **Winter Protocol** (`/winter-protocol`, WF-19, webhook `ad11ef02-14c8-4dbc-b1dd-bb5c9f3203bd`)
  - NOTE: the three remaining are guides rather than quizzes, so their pages will have simpler forms, but the change is the same three additions.
...
- [ ] **FlexOffers remaining (waiting on the FlexOffers rep, emailed July 29)**
  ...
  - [ ] Delete test contact **FlexOffers Test2** (`scott+fotest2@enyrgy.com`) after the postback test. Kept deliberately because it holds a test refid.

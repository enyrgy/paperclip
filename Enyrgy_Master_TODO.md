...
- [ ] **VERIFY WF-21 for the same false-consent bug** — expand its static Add Tag list (showed `type_consumer, source_lead_magnet, status_new, +3`). The Vitamin D Assessment page also collects phone and SMS consent conditionally. If `sms_consent_given` is in that static list, remove it (WF-21 already has a Dynamic Add Tag, so consent will then land correctly on its own).

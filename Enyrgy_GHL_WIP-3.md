...
| WF-35 fires on call events | Low | OPEN | Customer Replied is unfiltered by design, so calls and voicemails trigger it and `{{message.body}}` renders as `undefined`. **Do not fix by filtering out the call channel**, which would restore the silence described above. Branch on empty message body instead and send a call-specific notification. |

...
| Current investor import | COMPLETE (Session 16, July 31) | 57 imported, $10,373,000 funded. Tagged seg_investor_current + drip_bypass + source_investor_import; **never type_investor** (triggers the cold 8-touch Investor Drip). WF-01 **and WF-31** unpublished during import - WF-31 FlexOffers First Touch is a SECOND Contact Created trigger that post-dates the Session 10 audit. 46 created + 10 updated (dual-role legacy customers); 1 row failed silently and was added by hand. Smart list "Investors - Current". |
| Investor Spreadsheet (prospects) | In Progress | 100+ prospect contacts being entered. Separate from the 57 current investors above; keep prospects out of seg_investor_current. |
...
| Business line silently swallowed inbound calls | **High** | RESOLVED (Aug 7) | Forwarding to Scott's mobile was configured correctly, but Advanced Settings passed **the caller's** number through as caller ID, so unknown callers were silenced by iPhone's Silence Unknown Callers and never rang or appeared as missed. Any customer calling the number in every SMS footer was affected. Fixed by switching caller ID to **CRM's Phone Number** (888-316-1695). Found only because four spam voicemails produced four odd WF-35 notifications. |
| Inbound calls became consumer leads | Medium | RESOLVED (Aug 7) | GHL creates a contact per inbound call; WF-01 routed them to `Default Consumer`, so four robocalls became four `type_consumer` leads with Opportunities. Guard extended with `AND Email is not empty`. Routing an email-less contact does nothing anyway: no email means no drip send, no SMS Consent means WF-02's gate blocks the text. |
| GBP verification attracts scam robocalls | Low | ONGOING | The "your Google listing is not reaching customers" robocall targets recently verified Google Business Profiles. Enyrgy verified GBP in Session 15. Different numbers, identical voicemail. Do not call back or press a key to opt out. |
| WF-35 fires on call events | Low | RESOLVED (Aug 7) | Customer Replied is unfiltered by design, so calls and voicemails trigger it and `{{message.body}}` rendered as `undefined`. Fixed with a `Call or Voicemail` branch (`Replied message Is empty`) ordered **first**, sending a call-specific notification. The call channel was deliberately NOT filtered out: those useless notifications were the only thing that surfaced the silenced business line, so filtering would have restored that silence. Verified by a live call. |
...
- **Org + CEO Agent** created via the setup wizard (mission set; Claude Code adapter; env check passed). Canceled Paperclip's auto-fired demo task subtree ("Hire your first engineer" + Founding Engineer agent + 6 sub-tasks) so the workspace is clean.

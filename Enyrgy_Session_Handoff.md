...
Date of this handoff: **August 30, 2026 (end of Session 18)**
...
## WHAT WAS DONE (Session 18, August 26 to 30, 2026)

Almost entirely outward-facing documents plus one systems finding that matters more than any of them.

### Client and investor documents produced

| Document | For | Notes |
|---|---|---|
| `Enyrgy_Facility_Overview` v2.3 | Center for New Medicine, Irvine | Story, technology, the 12-week study in full, contraindications verbatim, then the commercial case. Built through `scripts/md2docx.py` against the brand reference. |
| `Enyrgy_Story_and_Clinical_Data` v1.1 | Data room | Origin story, the research foundation with citations, third-party spectral verification, the study with its published limitations, and the reading list as an appendix. |
| `Enyrgy_Investor_Followup_MonroeStreet` | Brian Goldberg, Monroe Street Capital Partners | Follow-up to the Aug 29 call introduced by Brian Cameron, carrying the DocSend data room link. |
| `Enyrgy_Letter_ZENA_Medical` v2.0 | Dr. Zena Gabriel, ZENA Medical, Newport Beach | Letter of introduction to an aesthetic dermatology practice. |
| `campaigns/summer-end-tired-test-outreach` | Millie | Phone script, voicemail and email for the 18 Tired Test prospects. |

**CNM commercial terms are bespoke and deliberately NOT in the KB.** $25 a session or $100 a week, and a 50/50 revenue share worth about $1,500 per consumer unit with no inventory. That is roughly 3.5x the top of the standard $49 to $125 operator range and 2.5x the standard $600. Scott's call to keep it out of an agent-read file entirely: an agent cannot quote terms it has never seen, which prevents propagation more simply than labeling the exception would. The terms live in the proposal document, which is where deal-specific numbers belong.

**The ZENA letter was rebuilt from scratch** after the first version was written around a premise that did not hold. A web source described the practice as offering Mohs surgery; their published menu is entirely aesthetic. Scott caught it. The real menu is a better hook, since much of what they sell exists to correct UVA damage.

**A citation was removed from the website.** The Gambichler 2002 reference under UVA-triggered serotonin production measures opioid peptides and does not mention serotonin, verified against Europe PMC. **Still open:** Kaur 2019 is now the only reference under that heading and it concerns serotonin's role in seasonal affective disorder rather than UVA producing it, so the heading still outruns its source. PMID 11952999 is the paper that was probably meant.

### Summer End offer

$500 off plus free shipping, code `SUMMEREND500`, **extended from Aug 31 to 11:59pm Pacific on September 15** because Scott, Brian and Millie are traveling the week of Sept 1. Enyrgy's first discount. The deadline is what protects the price and it should not move a second time.

**WF-12 now assigns Millie as owner**, so every new Tired Test lead arrives owned and WF-35 routes their replies to her. The 18 existing prospects were bulk-assigned. **The routing principle, worth keeping:** WF-35's `None` branch is a backstop, not a routing rule. It catches unowned replies across every funnel, so pointing it at one person routes work to someone whose remit does not cover it. Assignment is the layer to fix, because everything keys off it.

### Dealer program: two findings during the AVETTA Global LLC onboarding

**These are the substantive items from this session.** Full detail in `campaigns/dealer-onboarding.md`, which now opens with a stop notice.

**The per-dealer form does not isolate the `Dealer Name` dropdown.** Options live on the shared `opportunity.dealer_name` custom field, so duplicating the form gives each dealer their own URL and not their own list. Proven rather than suspected: setting the option on AVETTA's copy propagated to the shared field, and the master simultaneously lost `Unassigned`. **One dealer is safe. A second exposes both**, which is exactly the roster leak the structure exists to prevent. **Do not onboard dealer #2 until this is rebuilt.**

**Nothing set `Exclusivity Expires`, and WF-41 gates the dealer confirmation on it.** As built, every registration would have stalled unconfirmed. GHL cannot compute the date: its Update Opportunity date field offers a current date under `Right now` and no offset of any kind. Now set by hand at Conflict Check, with WF-41's empty check as the enforcement. The permanent fix is a Railway round trip using the abandoned-checkout service that already exists.

**WF-40 hardened.** A one-minute wait before the opportunity lookup, against the same async-write race that silently dropped FlexOffers refids on WF-31. An internal notification on the `Opportunity Not Found` branch, which previously ended in silence while the dealer believed they held a 90-day claim.

**AVETTA onboarding is at steps 1 to 5 of 7**, waiting on their W-9 before the registration link goes out and the test submission runs.

### Also this session

- **WF-12 sends no email at capture**, so the first thing a Tired Test lead hears is WF-13 touch 1 at Day 1, while WF-02's five-minute SMS tells them to check their email now. Either the page delivers the guide or a promise breaks at the first touch. Flagged in `campaigns/WF-13-tired-test.md`; check the page, and the other four magnets.
- `scripts/md2docx.py` gained `Facility Overview`, `Story and Clinical Data` and `Letter of Introduction` in its title map. Client letters carry no version stamp in the header, deliberately.

---

## WHAT WAS DONE (Session 17, around August 19 to 25, 2026)

**Reconstructed from commits and documents rather than from the session itself.** Detail lives in the Implementation Guide v3.9.7, the KB, and `campaigns/dealer-onboarding.md`.

- **The commercial dealer program was built.** WF-40 to WF-43 deal registration, the Commercial B2B pipeline stages, the Opportunity custom fields in the Dealer Registration folder, the 25 to 16 percent commission ladder against a $7,160 MAP floor, and `campaigns/dealer-onboarding.md`. KB Section 14b records the program with a standing instruction that agents never run a dealer conversation.
- **KB Section 16 added, contraindications and inbound health questions.** The verbatim list, the exemption from the prohibited-words scan, the photosensitizing-medication analysis, the vitamin D non-response caveat, the intact-skin rule, and the removal of the Heart/Pacemaker line.
- **Implementation Guide to v3.9.7** and **Enterprise Architecture to v1.2**, both closing roughly three weeks of accumulated drift.
- **KB corrections:** Scott's track record stated as two separate figures never to be combined, Brian Cameron as fractional CFO, the prevalence figure sourced to Cui 2023, the $600 facility referral commission, the $10M valuation, and the HSA/FSA buyer-of-record test.
...
- **The corporate-domain worry did not materialize.** All 20 corporate-domain investors delivered. The `scott@ledgerixpro.com` test that vanished without a bounce is therefore tenant-side quarantine (likely M365) on that one account: not the sending domain, not GHL suppression (Skipped was 0%), and not a signal about the investor list. Worth remembering as a diagnosis pattern - a message that vanishes with no bounce and no spam-folder entry is usually recipient-side quarantine, which is invisible to the sender.
...
**PRODUCT FEED: DECIDED AGAINST, August 16, 2026. Do not set one up until Shopify captures `refid`.** FlexOffers offers advertisers a product feed (Programs → Products) that publishes a product catalog so publishers can promote individual SKUs. It looks like an obvious win on the page and it is not, because it is incompatible with how 250377 attributes sales.

**Why it breaks.** Attribution is capture-and-postback: the five lead-magnet pages read `refid` from the landing URL and store it, and WF-30 fires the postback on Order fulfilled gated on `FlexOffers Refid is not empty`. A product feed hands publishers product-level links that go **straight to `shop.enyrgy.com`, which runs none of that capture code**. A visitor arrives on Shopify with a `refid` nothing reads, buys, and WF-30 finds the field empty and sends no postback. The publisher earns nothing having done exactly what the feed invited. This is the same failure shape as the WF-25 reply path: the tool invites an action the system cannot pay for. It is also why **Deep Linking is OFF** on this program. A product feed is a deep-linking mechanism, and the reason it was rejected then still holds.

**Conditions to revisit.** Either (a) add `refid` capture to the Shopify product pages and carry it into the order, after which direct product links attribute correctly and deep linking becomes viable generally, or (b) leave it. With **two SKUs** (`EN-UVB-020` and `ENRG 011`) a feed earns very little regardless; feeds pay off on large catalogs. Do NOT take the third option that suggests itself, pointing every feed `LinkUrl` at a lead magnet. Publishers expect a product page, get a quiz, and it reads as bait-and-switch.

**If it is ever built:** FlexOffers accepts CSV or XML, requires `AdvertiserId`, `Pid`, `Name`, `LinkUrl`, and accepts the same CSV used for the website, so `products_export_1.csv` is a starting point. The File Name convention is not in FlexOffers' public docs; it is on the SPECIFICATIONS tab of that page.
...
- Paperclip auto-fired a canned demo task ("Hire your first engineer"), which spawned a Founding Engineer agent and 6 sub-tasks. Stopped and canceled the entire subtree; the workspace is clean (0 tasks, 0 running agents). No net spend damage.
...
| Enyrgy_Facility_Overview | v2.3, Session 18 | .md + .docx | Prospect proposal for the Center for New Medicine. Story, technology, the 12-week study, contraindications verbatim, then the commercial case. **CNM's terms are bespoke and live only here, never in the KB.** |
| Enyrgy_Story_and_Clinical_Data | v1.1, Session 18 | .md + .docx | Data room paper. Origin story, research foundation with citations, third-party spectral verification, the study with published limitations, reading list appendix. **Keeps the five-participant study and the 25,000-session safety record explicitly apart**, since they answer different questions. |
| Enyrgy_Letter_ZENA_Medical | v2.0, Session 18 | .md + .docx | Letter of introduction to an aesthetic dermatology practice in Newport Beach. No version stamp in the header, deliberately: a version number on a letter says it is a template on its second pass. |
| Enyrgy_Investor_Followup_MonroeStreet | Session 18 | .md + .docx | Follow-up to Brian Goldberg carrying the data room link. |
| campaigns/dealer-onboarding | Session 17, revised Session 18 | .md + .docx | **Opens with a stop notice.** The per-dealer form does not isolate the dropdown, so no second dealer can be onboarded until the model is rebuilt. Also carries the W-9 request template and the manual expiry-date rule. |
| campaigns/summer-end-tired-test-outreach | Session 18 | .md + .docx | Millie's brief for the 18 Tired Test prospects. Written for someone starting from zero, with the build history deliberately stripped out. |
| campaigns/ (directory) | Session 16, Aug 3 | .md + .docx per workflow | **All 19 workflows, 94 touches, 83 corrections.** Every live email and SMS in GHL, with a per-workflow change log recording what was corrected and why. Plus `_INDEX`, `_LINKS` (canonical booking/form URLs), `_TEMPLATE` and `README`. GHL is the source of truth: change GHL first, then the file. Created because Standing Rule 9 did not exist when this copy was written. |
...
| Enyrgy_Paperclip_Knowledge_Base | v2, Session 16 | .md + .docx | Shared agent-read KB, 16 sections, voice-passed. **NEW Aug 24, 2026: Section 16, Contraindications and inbound health questions.** The contraindication list from the User Manual plus the rules for any conversation where a person names a condition, an injury, or a medication: never clear anyone, never treat absence from the list as clearance, never reason outward from it, do not record health details in GHL, escalate to Scott. **16.2** records the one standing exemption to the Section 11 prohibited-words scan, since the list needs "diagnosed" five times and scrubbing it breaks the disclosure. **16.3** is the substantive one: the app doses from Fitzpatrick skin type and cannot know what a user takes, so a photosensitizing drug puts the automatic cutoff above that person's burn threshold. The safety record is silent on this in both directions, because we never screened for those drugs or recorded them, so it must not be cited in answer to a photosensitivity question. **16.5** separates vitamin D genetic non-response from the safety list; the biology overlaps with the supplement low-responder figure without being identical (GC and CYP2R1 act by either route, DHCR7 governs skin substrate). **16.6** adds the intact-skin rule, absent from every Enyrgy document until now: our own approved research says vitamin D supports wound repair, so a reader who owns a UVB platform can conclude the light should go on the wound, and nothing published said otherwise. **16.7** records the removal of the Heart/Pacemaker line (Scott's decision, Aug 24) after research found no dermatological source supporting it; the 2019 AAD-NPF guidelines list neither implanted cardiac devices nor anything like them, and warnings of that shape trace to tanning-cabinet concerns about electromagnetic interference from ballasts and motors rather than from UV. Three constraints written in alongside: removal is not clearance, never volunteer it, never tell a customer holding the earlier printing that their manual is wrong. **The User Manual was revised to match** (EPP correction, pacemaker line removed, `Use on Intact Skin Only` subsection added) and saved by Scott as `Enyrgy Precision Wellness Platform User Manual v2.docx` in OneDrive. Units in the field carry the previous printing. **Brand Style Guide Section 08** now carries the same list plus the Intact Skin Notice; aligning it narrowed two consumer warnings, family history of skin cancer and breastfeeding, both recorded there as open. Pointers added at the KB header, Section 4 Safety and Section 11. **This section is NOT in the live Company Skill yet and the live agents cannot see it. Re-sync required.** **Live `enyrgy-knowledge-base` Company Skill last CONFIRMED synced Aug 3, 2026 at repo commit `42090226`. SUBSTANTIALLY DRIFTED SINCE, and an earlier version of this line understated it as "two edits", which was wrong.** Twenty-six commits have touched the KB since that sync, including two entire new sections. **Do not attempt a targeted find-and-replace against a list; run the diff.** `git log 42090226..HEAD -- Enyrgy_Paperclip_Knowledge_Base.md` and work from that. What has landed since, at minimum: **Section 16** contraindications and inbound health questions in full, **Section 14b** the commercial dealer program, the HSA/FSA Truemed rules including the buyer-of-record test, the prevalence figure moved from an unsourced 77 percent to 76.6 percent sourced to Cui 2023 with the never-say-adults rule, Scott's corrected track record and Brian Cameron as fractional CFO, the $600 facility referral commission, the $10M valuation, the take-home retail margin, and removal of the FDA general wellness guidance reference. **Whether Session 17 re-synced any of this is unrecorded**, which is itself the lesson: the synced commit hash goes in this table every time or the next session cannot tell what the live skill contains. Earlier syncs: Aug 2 at `2f4fb933`, Aug 1 at `95f41a40`. Six targeted edits applied (NOT a select-all paste): founder track record (S1), commercial pricing $49-$125 range + membership/credit-model guidance (S3), objection line $49-$125 (S11), PPM placeholder guardrail lifted (S12), distribution alongside white-label (S14), quick-lookup table row (S15). Earlier Session 14 additions (UVB waveband, sunlight five-outputs, payment options, participant-level pilot data, one-session-a-day) synced at `0062ce4e`. Upload to project. |
